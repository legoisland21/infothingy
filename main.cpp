...
void LoadSettings(bool& doAlert, int& colorId) {
    ifstream file2("settings.ini");
    string line;

    while (getline(file2, line)) {
        if (line.rfind("doAlert=", 0) == 0) {
            string val = line.substr(8);
            doAlert = (val == "1");
        } else if (line.rfind("colorId=", 0) == 0) {
            string val = line.substr(8);
            try {
                int parsed = stoi(val);
...
void PrintSettings() {
	LogToFile("Opened Settings");
	system("cls");
	cout << "Choose a option" << endl;
	cout << "1. Change color (MATRIX, CLASSIC XP, NORMAL)" << endl;
	cout << "2. Alerts (ON, OFF)" << endl;
	cout << "3. Return" << endl;
	cout << "0. Reset Settings" << endl;
	cin >> opt;
	if(opt == 1) {
		colorid++;
		if(colorid == 3) {
...
int PrintMenu() {
	...
	if(opt == 8) {
		...
		file << "doAlert="+to_string(doAlert)<<endl;
		file << "colorId="+to_string(colorid)<<endl;
		PrintSettings();
		file << "doAlert="+to_string(doAlert)<<endl;
		file << "colorId="+to_string(colorid)<<endl;
	}
